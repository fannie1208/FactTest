    ...
    if len(unique_generated_texts) > 1:
        ...
        for i, reference_answer in enumerate(unique_generated_texts):
            for j in range(i + 1, len(unique_generated_texts)):
                ...
                prediction = deberta_model(torch.tensor(torch.tensor([encoded_input]), device=device))['logits']
                predicted_label = torch.argmax(prediction, dim=1)

                reverse_input = qa_2 + ' [SEP] ' + qa_1
                encoded_reverse_input = deberta_tokenizer.encode(reverse_input, padding=True)
                reverse_prediction = deberta_model(torch.tensor(torch.tensor([encoded_reverse_input]), device=device))['logits']
...
if __name__ == "__main__":
    ...
    if args.stored:
        with open(f"../training_data/Hotpot_{args.dataset}_{args.num_try}_{model_name}_semantic_certainties.json",'r') as f:
        ...
        with open(f"../training_data/Hotpot_{args.dataset}_{args.num_try}_{model_name}_semantic_certainties.json",'w') as f:
