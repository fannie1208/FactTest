...
def eval_ap(results, sure_prob=False):
    if sure_prob:
        sorted_data = sorted(results, key=lambda x: x[-2], reverse=True)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--method",type=str,default="vanilla",choices=["vanilla","semantic","rtuning","classifier","scgpt"])
    ...
    print("pretrained AP (use sure prob)", eval_ap(data, sure_prob=True))
