...
def extract_features(model, tokenizer, input_data, layer_idx=16):
...
if __name__ == "__main__":
    ...
    sample_feature = extract_features(model, tokenizer, data[0], args.layer_idx)
    ...
    with accelerator.split_between_processes(data) as data:
        ...
        for sample in tqdm(data):
            ...
            is_correct = sample['answer'].lower() in output.lower()
            
            # Extract features for the classifier
            feature = extract_features(model, tokenizer, sample, args.layer_idx)
    ...
    if accelerator.is_main_process:
        ...
        with open(f"results/ours_saplma_{model_name}.json", 'w') as f:
            json.dump(results, f)
        ...
        print(f"Evaluation completed. Results saved to results/ours_saplma_{model_name}.json")
